...
def remind_day_kb():
    """Кнопки выбора дня: сегодня, завтра, через 2/3/7 дней"""
    return InlineKeyboardMarkup(inline_keyboard=[
        [
...
def remind_time_kb():
    """Кнопки выбора времени с 05:00 до 23:00"""
    rows = []
    hours = list(range(5, 24))  # 05, 06, 07, ... 23
    # По 3 кнопки в ряд
...
def focus_task_list_kb(tasks):
    ...
    for task in tasks:
        buttons.append([
            InlineKeyboardButton(
                text=task["title"],
                callback_data=f"focus_start:{task['id']}"
            )
        ])
