...
from keyboards import (
    ...
    task_day_kb, evening_audit_kb, settings_kb
)
...
async def handle_task_day(message: Message, state: FSMContext):
    ...
    if not task:
        await message.answer(
            ...
            "Напиши что хочешь сделать сегодня:",
            parse_mode="HTML"
        )
    ...
    await message.answer(
        ...
        reply_markup=task_day_kb(has_task=True),
        parse_mode="HTML"
    )
...
async def cb_after_review(callback: CallbackQuery):
    ...
    if not task:
        await callback.message.answer(
            "📋 Задача на сегодня не поставлена. Напиши что хочешь сделать:"
        )
        return
    task_text = task["main_task"] or "не указана"
    completed = task["completed"]
